...
new CvarEndMapMaxDrop,EndMapMaxDrop;
...
public plugin_init()
{
	...
	CvarEndMapMaxDrop= register_cvar("csgo_vipbonus", "5");
	...
	EndMapMaxDrop = get_pcvar_num(CvarEndMapMaxDrop);
	...
	set_task(5.0,"LosujSkrzynie",_,_,_,"d");
...
{
	client_cmd(id, "mp3 play sound/csgo/start.mp3");
	return 0;
}
...
public cmd_Help(id)
{
	show_motd(id, "addons/amxmodx/configs/csgo_donaciak/help.html", "Help CS:GO Mod");
	return PLUGIN_CONTINUE;
}

public cmd_Commands(id)
{
	show_motd(id, "addons/amxmodx/configs/csgo/komendy.html", "Commands CS:GO Mod");
	return PLUGIN_CONTINUE;
}
...
public CM_Opcje(id, menu, item)
{
	...
	case 0:
        {
            if(Dollars[id] >= CasePrice* iilosc){
...
public AdminCMD(id)
{
    new admin_steam[40],pid_steam[40];
    get_user_authid(id,admin_steam,charsmax(admin_steam));	
    get_user_authid(pid,pid_steam,charsmax(pid_steam));	
    ...
    if(iliczba1>0)
	{
		if(tryb[id] == 1)
		{
			...
			log_to_file("addons/amxmodx/logs/csgo_admin.log", "Admin [%s-%s] Przekazal %u Kluczy dla  %s-%s  ^n",  Name[id], admin_steam,iliczba1,Name[pid],pid_steam);
		}
		if(tryb[id] == 2)
		{
			CaseGiveMenu(id);
		...
		if(tryb[id] == 3)
		{
			...
			tryb[id] = 0;
			set_task(0.5, "SQL_UpdateUser", pid);
			log_to_file("addons/amxmodx/logs/csgo_admin.log", "Admin [%s-%s] Przekazal %u Dollarow dla  %s-%s  ^n",  Name[id], admin_steam,iliczba1,Name[pid],pid_steam);
		}
	}
}
...
{
	new String[128];
	formatex(String, charsmax(String), "%L", id, "CASEGIVEM", Name[pid]);
	new Menu = menu_create(String, "CaseMenuhG2");
	
	for(new i = 1; i < sizeof(Case_Data); i++)
	{
		new NumToString[6];
		num_to_str(i, NumToString, 5);
		formatex(String, charsmax(String), "%s \r(%d)", Case_Data[i][0], Cases[pid][i]);
		menu_additem(Menu, String, NumToString);
	}
...
public CaseMenuhG2(id, Menu, Item)
{
	if(Item == MENU_EXIT)
	{
		menu_destroy(Menu);
	...
	new x = str_to_num(Data);
	Cases[pid][x]+=iliczba1;
	log_to_file("addons/amxmodx/logs/csgo_admin.log", "Admin [%s-%s] Przekazal %u Skrzynki %s dla  %s-%s  ^n",  Name[id], admin_steam,iliczba1,Case_Data[x][0][0],Name[pid],pid_steam);
	CaseGiveMenu(id);
}
//
...
public client_infochanged(id)
{
	new szNowyNick[32];
	new szStaryNick[32];
	get_user_info(id, "name", szNowyNick, 31);
	get_user_name(id, szStaryNick, 31);
	if (!is_user_connected(id) || is_user_bot(id))
	{
		return 0;
	}
	if (!equal(szNowyNick, szStaryNick))
	{
		set_user_info(id, "name", szStaryNick);
		print_color(id, "!g%s!y %L", Prefix, id, "NICKCHANGE");
		return 1;
	}
	return 0;
}
public LosujSkrzynie()
{
	new RandomNum;
	RandomNum = random(1001);
	new RandomPL = random(EndMapMaxDrop);
	for(new i = 1; i < sizeof(Case_Data); i++)
	{
		if(Case_Data[i][1][0] <= RandomNum)
		{
			for(new x=1; x<=RandomPL; x++)
				if(is_user_connected(x))
				{
					Cases[x][i]++;
					print_color(0, "!g%s!y %L", Prefix, 0, "FOUND", Name[x], Case_Data[i][0]);
					SQL_UpdateUser(x);
				}
			break;
		}
...
{
		register_native("csgo_login", "nat_Zalogowany", 1);
		//get
		register_native("csgo_get_user_dollars", "nat_PobierzDollary", 1);
		register_native("csgo_get_user_keys", "nat_PobierzKlucze", 1);
		register_native("csgo_get_user_cases", "nat_PobierzSkrzynie", 1);
		register_native("csgo_get_user_kills", "nat_PobierzKille", 1);
		register_native("csgo_get_user_rank", "nat_PobierzRange", 1);
		//set
		register_native("csgo_set_user_dollars", "nat_UstawDollaryGracza", 1);
		register_native("csgo_set_user_keys", "nat_UstawKluczeGracza", 1);
		register_native("csgo_set_user_cases", "nat_UstawSkrzynieGracza", 1);
		register_native("csgo_set_user_kills", "nat_UstawKilleGracza", 1);
		
}
public nat_Zalogowany(id)
{
	return Logined[id];
}
public nat_PobierzDollary(id)
{
	return Dollars[id];
}
public nat_PobierzKlucze(id)
{
	return Keys[id];
}
public nat_PobierzSkrzynie(id,cid)
{
	return Cases[id][cid];
}
public nat_PobierzKille(id)
{
	return Kills[id];
}
public nat_PobierzRange(id)
{
	return CurrentRank[id];
}
public nat_UstawDollaryGracza(id, iDollar)
{
	Dollars[id] = iDollar;
	return PLUGIN_CONTINUE;
}
public nat_UstawKluczeGracza(id, iKey)
{
	Keys[id] = iKey;
	return PLUGIN_CONTINUE;
}
public nat_UstawSkrzynieGracza(id, cid ,iIlosc)
{
	Cases[id][cid] = iIlosc;
	return PLUGIN_CONTINUE;
}
public nat_UstawKilleGracza(id, iKill)
{
	Kills[id] = iKill;
	return PLUGIN_CONTINUE;
}
